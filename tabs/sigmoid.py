...
def app():
    ...
    if selected == "Description":
        st.markdown('''<p style="font-size:20px; text-align:justify">The sigmoid activation function is a common non-linear activation function used in artificial neural networks and machine learning models. It maps any real-valued number to a value between 0 and 1. The sigmoid activation function is commonly used in the output layer of binary classification models, where the goal is to predict binary labels (0 or 1) or estimate probabilities. It was historically used as the activation function in hidden layers of shallow neural networks, but it has largely been replaced by other activation functions like ReLU in deep networks due to the vanishing gradient problem. In brief, the sigmoid activation function is a foundational component of neural networks and is still relevant for specific tasks, especially in binary classification. However, its use in hidden layers of deep networks has become less common due to issues like the vanishing gradient problem and the prevalence of other activation functions that offer better training dynamics.</p>''', unsafe_allow_html=True)
        
    if selected=="Explanation":
        st.write("The sigmoid function has a characteristic S-shaped curve and is defined as follows:")
        st.latex(r'''Sigmoid(x) = \frac{1}{1 + e^{-x}}''')
                
        st.markdown('Where:<br><br><ul><li>$Input$ $Transformation$ $(x)$: The input to the sigmoid function, denoted as $x$, can be any real number. It represents the weighted sum of input features and biases in a neural network layer before the activation. </li><br><li>$Exponential$ $Component$ $(e^-x):$ : In the formula, the input $x$ is negated and exponentiated using the constant $e$, which is the base of the natural logarithm. The exponential component produces values between 0 and 1 for any real $x$.</li><br><li>$Denominator$ $(1 + e^-x)$: The exponential component is added to 1, resulting in the denominator of the fraction. This ensures that the sigmoid function\'s output is a value between 0 and 1.</li><li><br>$Final$ $Output$ $(Sigmoid(x))$: he final output of the sigmoid function is obtained by taking the reciprocal (1 divided by the denominator). The sigmoid function maps the input $x$ to a value between 0 and 1. When $x$ is very large, the output is close to 1, and when $x$ is very small (large negative), the output is close to 0.</li></ul>',unsafe_allow_html=True)
    ...
    if selected == "Implementation":
        ...
        st.code('''
                # Sigmoid Activation Function Pseudocode
                function sigmoid(x):
                    # Calculate the exponential component
                    exp_component = exp(-x)
                    
                    # Calculate the denominator
                    denominator = 1 + exp_component
        ...
        st.markdown('''<p style="font-size:17px;text-align:justify;">We define a simple neural network (SimpleNet) with two fully connected layers and a sigmoid activation layer. The forward method specifies the forward pass of the network, applying the sigmoid activation after the first fully connected layer. We create an instance of the neural network (model). We define a sample input (input_data) with shape (1, 10). We perform a forward pass to obtain the output. We define a loss function (mean squared error) and an optimizer (Stochastic Gradient Descent) for training. Note that this example does not include a full training loop, which would involve iterating over a dataset, computing gradients, and updating the model's parameters. In practice, sigmoid activation functions are used in various neural network architectures, especially in the output layer for binary classification tasks, where the sigmoid function's output can be interpreted as class probabilities.</p>''',unsafe_allow_html=True)
    ...
    if selected == "Visualization":
        ...
        # Streamlit app
        st.subheader("Sigmoid Activation Function Visualization")
...
# Create a range of x values
        ...
        with col1:
            ...
            a = st.slider("Parameter a", 0.01, 10.0, 1.0)
        ...
        with col2:
            # Create a range of x values
            # Compute the sigmoid activation function values
            sigmoid_values = sigmoid(a * x)

            # Create the plot
            fig, ax = plt.subplots(figsize=(8, 5))
            ax.plot(x, sigmoid_values, label=f'Sigmoid (a={a})', color=colour,linewidth=thickness)
            ax.set_xlabel('x')
            ax.set_ylabel('Sigmoid(ax)')
